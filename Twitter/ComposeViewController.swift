        ...
        if tweetTextView.text.isEmpty {
            ...
            if replyToUser == nil {
                ...
                TwitterClient.sharedInstance.postStatus(text: "\(replyToUserScreenName) \(tweetTextView.text!)", inReplyToStatusID: tweet.IDString!, success: { (tweet: Tweet) in
                    ...
                    if let replyID = tweet.inReplyToStatusIDStr {
//                        RKDropdownAlert.title("Success!", message: "ReplyID is \(replyID)\nTap to Dismiss", backgroundColor: TwitterClient.Colors.successBackgroundColor, textColor: TwitterClient.Colors.successTextColor, time: 1, delegate: self)
                    } else {
//                        RKDropdownAlert.title("Success!", message: "Tap to Dismiss", backgroundColor: TwitterClient.Colors.successBackgroundColor, textColor: TwitterClient.Colors.successTextColor, time: 1, delegate: self)
                    }
...
//                    RKDropdownAlert.title("Error! \(error.localizedDescription)", message: "Please Try Again Later", backgroundColor: TwitterClient.Colors.failureBackgroundColor, textColor: TwitterClient.Colors.failureTextColor, time: 1)
    ...
    fileprivate func configurePlaceholder() {
        if replyToUser == nil { // regular compose
